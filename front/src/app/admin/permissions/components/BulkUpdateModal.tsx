...
import type { BulkUpdateItem } from '@/types/permission';
...
export const BulkUpdateModal = ({ open, onClose, onSave, isSaving }: BulkUpdateModalProps) => {
                ...
                <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div className="p-6 space-y-3">
                        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                            <p className="text-xs text-blue-700 dark:text-blue-300">O'zgartirish mumkin: resource, action, description, category, scope, apiPath, frontendPath, uiKey, isSystemPermission</p>
                        </div>
                        <textarea value={raw} onChange={(e) => setRaw(e.target.value)} rows={12} className="w-full px-3 py-2.5 text-sm font-mono border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 resize-none" />
